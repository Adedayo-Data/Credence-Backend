...
describe('DB repositories integration', () => {
  ...
  describe('identities repository', () => {
    it('supports CRUD and list query', async () => {
      ...
      assert.equal(await identitiesRepository.findByAddress(second.address), null)
    })
    ...
    it('enforces unique and non-empty address constraints', async () => {
      ...
      await expectPgError(identitiesRepository.create({ address: '   ' }), '23514')
    })
  })
  ...
  describe('bonds repository', () => {
    it('supports CRUD and identity query methods', async () => {
      ...
      const updated = await bondsRepository.updateStatus(created.id, 'released')
      assert.ok(updated)
  ...
  describe('slash events repository', () => {
    it('supports CRUD, list and aggregate query methods', async () => {
      ...
      assert.equal(await slashEventsRepository.totalSlashedForBond(999_999), '0')
  ...
  describe('score history repository', () => {
    it('supports create, list, latest, and delete methods', async () => {
      ...
      const listed = await scoreHistoryRepository.listByIdentity('GSCORE_OWNER')
      assert.deepEqual(
        listed.map((entry) => entry.id),
        [newer.id, older.id]
      )

      const latest = await scoreHistoryRepository.findLatestByIdentity('GSCORE_OWNER')
      assert.ok(latest)
      assert.equal(latest.id, newer.id)
      assert.equal(latest.score, 80)

      assert.equal(await scoreHistoryRepository.findLatestByIdentity('UNKNOWN'), null)
  ...
  describe('cross-table foreign key behavior', () => {
    it('cascades dependent rows from identities to all child tables', async () => {
      ...
      assert.equal(await attestationsRepository.findById(attestation.id), null)
      assert.equal(await slashEventsRepository.findById(slashEvent.id), null)
      assert.deepEqual(await scoreHistoryRepository.listByIdentity('GCASCADE_OWNER'), [])
      assert.notEqual(
  ...
  describe('edge cases and additional coverage', () => {
    it('handles empty results gracefully', async () => {
      ...
      assert.deepEqual(await attestationsRepository.listBySubject('NONEXISTENT'), [])
      ...
      assert.deepEqual(await scoreHistoryRepository.listByIdentity('NONEXISTENT'), [])
      ...
      assert.equal(await scoreHistoryRepository.findLatestByIdentity('MISSING'), null)
    })
    ...
    it('handles special string characters and nulls', async () => {
      ...
      await identitiesRepository.update('GNULL_TEST', { displayName: 'Not null anymore' })
    ...
    it('tests all score sources and bond statuses', async () => {
      ...
      assert.ok(sources.every(source => history.some(entry => entry.source === source)))
    ...
    it('tests timestamp and date handling', async () => {
      ...
      const orderedHistory = await scoreHistoryRepository.listByIdentity('GDATE_TEST')
